...
with st.sidebar:
    st.subheader('Streamlit App by [Rafaella Duarte](https://www.linkedin.com/in/rafaella-duarte-carvalho/)')
    # base = st.sidebar.selectbox('Selecione a base de dados',options=('IMBd','TMBd'))
    st.markdown('+ **IMDb:** conhecida como Internet Movie Database, é uma base de dados online de informação sobre cinema TV, música e games, hoje pertencente à Amazon.Nesta base há cerca de 100836 registros referente as votações dos filmes e 9742 registros na base referentes aos filmes.')
    st.markdown('+ **TMDb:** conhecida como The Movie Database, é uma base de dados grátis e de código aberto,sobre filmes e seriados, criado por Travis Bell em 2008. Atualizado constantemente através do apoio da comunidade. Inicialmente, era apenas uma base de dados sobre filmes, mas em 2013 foi adicionada a seção de séries. Nesta base há cerca de 4803 registros referentes aos filmes e votações.')
...
with row1_1:
    st.markdown('Prazer, meu nome é Rafaella Duarte sou graduanda em Ciências Econômicas, entusiasta em Big Data e apaixonada por processamento e infraestrutura de dados. Dessa forma, a fim de estudar a linguagem de programação python e suas principais bibliotecas utilizadas para analise de dados. Desenvolvi este projeto de análise exploratória utilizando duas base de dados sobre filmes.')
    st.markdown("Você pode encontrar o código-fonte no [Film Rating Exploratory Analysis GitHub Repository](https://github.com/rafaelladuarte/film_rating_exploratory_analysis)")
